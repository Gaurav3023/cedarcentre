...
import { Trees, Compass, Heart, LogOut, Sun, Cloud, Book, PlayCircle, Star, Sparkles, ChevronRight, MessageCircle, X, Send, Clock, CheckCircle, Menu, AlertCircle, Calendar, LayoutGrid, List, Lock } from "lucide-react";
...
function StudentDashboardContent() {
  ...
  useEffect(() => {
    const savedMode = localStorage.getItem('cedar_view_mode') as 'grid' | 'list';
    if (savedMode) {
      setViewMode(savedMode);
    } else {
      setViewMode('grid');
      localStorage.setItem('cedar_view_mode', 'grid');
    }
  }, []);
        ...
        <header className="px-6 md:px-12 py-5 flex justify-between items-center bg-white/80 backdrop-blur-md sticky top-0 z-[100] border-b border-slate-100 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:block">
               ...
               <p className="font-serif italic text-xs text-slate-400 mt-1">STAIR Program Platform</p>
            </div>
          </div>
                          ...
                          <div key={weekNum} className="space-y-10">
                             ...
                             <div className={viewMode === 'grid' ? "grid grid-cols-1 md:grid-cols-2 gap-8" : "space-y-4"}>
                                {group.lessons.map((lesson, idx) => {
                                   ...
                                   const isLocked = lesson.releaseDate ? new Date(lesson.releaseDate) > new Date() : false;
                                   const isOverdue = lesson.deadline && !isCompleted && !isLocked && new Date(lesson.deadline) < new Date();
                                   const isDueSoon = lesson.deadline && !isCompleted && !isLocked && !isOverdue && (new Date(lesson.deadline).getTime() - new Date().getTime()) < 24 * 60 * 60 * 1000;
                                   ...
                                   return viewMode === 'grid' ? (
                                     <motion.div key={lesson.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }} className={`p-10 rounded-[3.5rem] border shadow-premium hover:shadow-premium-hover transition-all group flex flex-col relative overflow-hidden ${isCompleted ? 'bg-slate-50/50 border-slate-100 grayscale opacity-60' : isLocked ? 'bg-slate-50 border-slate-200 grayscale opacity-60' : `${colorClass.split(' ')[0]} ${colorClass.split(' ')[1]}`}`}>
                                         {!isRead && !isCompleted && !isLocked && <div className="absolute top-0 left-0 p-5 bg-cedar-primary text-white text-[8px] font-black uppercase tracking-widest rounded-br-[2rem] z-20 shadow-lg">New</div>}
                                         {isLocked && <div className="absolute top-0 left-0 p-5 bg-slate-700 text-white text-[8px] font-black uppercase tracking-widest rounded-br-[2rem] z-20 shadow-lg flex items-center gap-2"><Lock className="w-3 h-3" /> Locked</div>}
                                        ...
                                        <div className="flex justify-between items-start mb-8">
                                           <div className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-transform group-hover:scale-110 bg-white shadow-sm ${isOverdue ? 'text-red-500' : isLocked ? 'text-slate-400' : colorClass.split(' ')[2]}`}>{lesson.type === 'video' ? <PlayCircle className="w-8 h-8" /> : <Book className="w-8 h-8" />}</div>
                                        ...
                                        <h4 className={`text-2xl font-bold mb-3 ${isCompleted || isLocked ? 'text-slate-400' : 'text-slate-800'}`}>{lesson.title}</h4>
                                        {/* Description Removed as per request */}
                                        <div className="mt-auto pt-8 border-t border-slate-200/50 flex justify-between items-center">
                                           <div className="space-y-1">
                                             {isLocked && lesson.releaseDate ? (
                                                <div className="flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-slate-400"><Lock className="w-3.5 h-3.5" /> Unlocks: {new Date(lesson.releaseDate).toLocaleDateString([], { month: 'short', day: 'numeric' })} at {new Date(lesson.releaseDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                                             ) : (
                                                lesson.deadline && !isCompleted && <div className={`flex items-center gap-2 text-[9px] font-black uppercase tracking-widest ${isOverdue ? 'text-red-600' : 'text-slate-400'}`}><Clock className={`w-3.5 h-3.5 ${isOverdue ? 'text-red-600' : 'text-amber-400'}`} /> {isOverdue ? 'Critical Delay' : `Due: ${new Date(lesson.deadline).toLocaleDateString([], { month: 'short', day: 'numeric' })} at ${new Date(lesson.deadline).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`}</div>
                                             )}
                                           </div>
                                           {isLocked ? (
                                     ...
                                     <motion.div key={lesson.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.05 }} className={`p-6 rounded-[2rem] border transition-all flex items-center justify-between group ${isCompleted ? 'bg-slate-50/50 border-slate-100 grayscale opacity-60' : isLocked ? 'bg-slate-50 border-slate-200 grayscale opacity-60' : 'bg-white border-slate-100 hover:border-cedar-primary/20 shadow-sm hover:shadow-md'}`}>
                                        <div className="flex items-center gap-6">
                                           <div className={`w-14 h-14 rounded-xl flex items-center justify-center shadow-inner ${isCompleted ? 'bg-slate-100 text-slate-400' : isLocked ? 'bg-slate-200 text-slate-400' : 'bg-slate-50 text-cedar-primary'}`}>{lesson.type === 'video' ? <PlayCircle className="w-6 h-6" /> : <Book className="w-6 h-6" />}</div>
                                           <div>
                                              <div className="flex items-center gap-3 mb-1">
                                                 <h5 className={`font-bold text-sm ${isCompleted || isLocked ? 'text-slate-400' : 'text-slate-800'}`}>{lesson.title}</h5>
                                                 {!isRead && !isCompleted && !isLocked && <span className="w-2 h-2 bg-cedar-primary rounded-full" />}
                                        ...
                                        <div className="flex items-center gap-4">
                                           {isLocked && lesson.releaseDate ? (
                                              <div className="hidden sm:flex flex-col items-end">
                                                 <p className="text-[8px] font-black uppercase tracking-widest text-slate-400">Unlocks {new Date(lesson.releaseDate).toLocaleDateString([], { month: 'short', day: 'numeric' })}</p>
                                              </div>
                                           ) : isCompleted ? (
                                           ...
                                           {isLocked ? (
                                              <div className="p-3 rounded-xl transition-all bg-slate-100 text-slate-300 cursor-not-allowed"><Lock className="w-4 h-4" /></div>
                                           ) : (
                     ...
                     <div className="flex-1 flex flex-col">
                        <div className="flex items-center gap-6 mb-8">
                           ...
                           <div>
                              ...
                              <p className="text-[9px] text-slate-400 font-black uppercase tracking-widest mt-1">Directly with your coach</p>
                           </div>
                        </div>
                        <div className="flex-1 space-y-6 overflow-y-auto max-h-[350px] mb-8 pr-4 custom-scrollbar">
                           ...
                           {supportRequests.find(r => r.id === selectedRequestId)?.chat?.map((msg, i) => {
                             ...
                             const senderName = isStudent ? user?.name : (users.find(u => u.id === msg.senderId)?.name || 'Coach');
