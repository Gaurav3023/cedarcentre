    ...
    <div className="flex flex-col h-full p-8">
      ...
      <div className="mt-auto pt-8 border-t border-slate-100">
        <div className="bg-slate-50 p-6 rounded-[2rem] mb-6 flex items-center gap-4">
           ...
           <div className="flex-1 min-w-0"><p className="font-bold text-slate-800 text-sm truncate">{user?.name}</p><p className="text-[10px] text-slate-400 font-medium uppercase tracking-widest">Coach Portal</p></div>
        </div>
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <main className="flex-1 p-6 lg:p-12 overflow-y-auto w-full">
            ...
            <header className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
              <div>
                <p className="text-[10px] font-black uppercase text-cedar-primary tracking-[0.3em] mb-3">Coach Portal</p>
                ...
                <p className="text-slate-400 mt-4 font-medium tracking-tight">
                  {activeTab === 'dashboard' ? 'Here is what is happening in your learning community today.' : 
                   ...
                   activeTab === 'students' ? 'Track progress and engagement for all participants.' :
                   activeTab === 'submissions' ? 'Review submissions and award star points.' :
                   'Direct participant support and case management.'}
                </p>
              </div>
              {activeTab === 'lessons' && (
                <div className="flex items-center gap-4">
                  ...
                  <button onClick={() => { resetLessonForm(); setLessonModal({ mode: 'create' }); }} className="flex items-center gap-3 bg-cedar-primary text-white px-8 py-4 rounded-[2rem] font-bold text-sm shadow-xl shadow-cedar-primary/20 hover:scale-105 transition-all w-fit"><Plus className="w-5 h-5" /> Create STAIR Program Step</button>
                </div>
              )}
          </header>
            ...
            {activeTab === 'dashboard' && (
              <motion.div key="dashboard-tab" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-12">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                  <StatCard icon={<Users />} label="Active Participants" value={stats.students} color="bg-blue-50 text-blue-500" />
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-10">
                   <div className="xl:col-span-2 space-y-8">
                      <div className="flex items-center justify-between"><h3 className="text-2xl font-serif text-slate-800 italic">Recent Participant Activity</h3><button onClick={() => setActiveTab('submissions')} className="text-xs font-black uppercase text-cedar-primary tracking-widest flex items-center gap-2 hover:gap-3 transition-all">View All <ArrowRight className="w-4 h-4" /></button></div>
                      ...
                      <div className="bg-white p-10 rounded-[3.5rem] border border-slate-100 shadow-sm space-y-8">
                         ...
                         <div className="space-y-4">
                            <button onClick={() => { resetLessonForm(); setLessonModal({ mode: 'create' }); setActiveTab('lessons'); }} className="w-full flex items-center justify-between p-5 bg-slate-50 hover:bg-cedar-primary hover:text-white rounded-3xl transition-all group">
                               <span className="font-bold text-sm">Create STAIR Program Step</span><Plus className="w-5 h-5 text-cedar-primary group-hover:text-white" />
                            </button>
                   ...
                   <motion.div key={req.id} onViewportEnter={() => req.educatorHasUnread && markSupportAsRead(req.id, 'educator')} className={`bg-white p-8 rounded-[4rem] border transition-all flex flex-col gap-8 shadow-sm relative ${req.educatorHasUnread ? 'ring-2 ring-red-500/10 border-red-100' : 'border-slate-50'}`}>
                      ...
                      <div className="space-y-6">
                         ...
                         {req.chat?.map((msg, i) => {
                           ...
                           const senderName = users.find(u => u.id === msg.senderId)?.name || (isEducator ? user?.name : 'Participant');
              ...
              <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-[4rem] w-full max-w-4xl p-12 shadow-2xl relative flex flex-col max-h-[90vh]">
                 ...
                 <div className="flex-1 overflow-y-auto pr-4 custom-scrollbar">
                    <h4 className="text-sm font-black uppercase text-slate-400 tracking-widest mb-8 px-2 flex items-center gap-3">
                      <GraduationCap className="w-5 h-5" /> Assigned STAIR Program Path
                    </h4>
                    <div className="space-y-4">
                      {(() => { 
        ...
        {lessonModal && (
          <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-md z-[400] flex items-center justify-center p-6">
             <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-[4rem] w-full max-w-4xl p-12 lg:p-16 shadow-2xl relative max-h-[90vh] overflow-y-auto custom-scrollbar">
                ...
                <div className="mb-12"><h3 className="text-4xl font-serif text-slate-800 italic">{lessonModal.mode === 'create' ? 'Create STAIR Program Step' : 'Refine Journey Step'}</h3><p className="text-slate-400 font-medium mt-2">Design an impactful learning experience for your participants.</p></div>
                      ...
                      <div className="space-y-8 bg-slate-50/50 p-8 rounded-[3rem] border border-slate-100">
                        ...
                        <div className="space-y-6">
                           <div className="flex p-1 bg-white border border-slate-100 rounded-2xl">
                              ...
                              <button type="button" onClick={() => setLessonForm({...lessonForm, assignedStudents: []})} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${!lessonForm.assignedStudents.includes('all') ? 'bg-cedar-primary text-white shadow-lg' : 'text-slate-400'}`}>Specific Participants</button>
                           </div>
