            ...
            {activeTab === 'support' && (
              <motion.div key="support-tab" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-8">
                 {mySupportRequests.filter(r => r.status === 'open' || r.educatorHasUnread).map(req => (
                   <div key={req.id} onViewportEnter={() => req.educatorHasUnread && markSupportAsRead(req.id, 'educator')} className={`bg-white p-8 rounded-[4rem] border transition-all flex flex-col gap-8 shadow-sm relative ${req.educatorHasUnread ? 'ring-2 ring-red-500/10 border-red-100' : 'border-slate-50'}`}>
